    ...
    public function viewProfileById(Request $request, $id)
    {
        try {
            ...
            if ($loggedInUser) {
                ...
                $user->isFollowing = OrganizerFollow::where('participant_user_id', $loggedInUser->id)->where('organizer_user_id', $user->id)->first();
            } else {
                // @phpstan-ignore-next-line
                $user->isFollowing = null;
            }
    ...
    public function editProfile(UpdateOrganizersRequest $request)
    {
        ...
        try {
            DB::transaction(function () use ($user, $validatedData) {
                ...
                if ($validatedData['userProfile']['name']) {
                    $user->name = $validatedData['userProfile']['name'];
                }
    ...
    private function viewProfile(Request $request, $logged_user_id, $userProfile, $isOwnProfile = true)
    {
        try {
            ...
            $tierPrizeCount = DB::table('event_details')
                ...
                ->select(['event_details.id as event_id', 'event_details.event_tier_id', 'event_tier.tierPrizePool'])
                ->sum('tierPrizePool');
            ...
            ['joinEvents' => $joinEvents, 'activeEvents' => $joinEventsActive, 'historyEvents' => $joinEventsHistory] = EventDetail::processEvents($joinEvents, $isFollowing);

            return view('Public.OrgProfile', compact('joinEvents', 'userProfile', 'isOwnProfile', 'followersCount', 'joinEventsHistory', 'joinEventsActive', 'followCounts', 'lastYearEventsCount', 'beforeLastYearEventsCount', 'teamsCount', 'tierPrizeCount'));
        } catch (Exception $e) {
            return $this->showErrorParticipant($e->getMessage());
        }
    }
