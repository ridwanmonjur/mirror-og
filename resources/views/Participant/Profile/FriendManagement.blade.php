@auth
    ...
    <div class="d-flex justify-content-start align-items-center dropdown-show">
    ...
    @if ($friend)
        @if ($friend->status == 'pending')
            ...
            @else
                <span style="font-size: 16px; font-weight: lighter;" class="me-3">
                ...
                <button type="button"
                    ...
                    class="btn bg-primary text-light px-4 py-2 me-2 rounded-pill">
                    <span>Accept</span>
                </button>
                <button type="button"
                    onclick="formRequestSubmitById('Are you sure you want to reject this friend request?' ,'rejectFriendRequest')"
                    class="btn border-danger text-red px-4 py-2 me-2 rounded-pill">
        ...
        @elseif ($friend->status == 'rejected')
            <button type="button" style="background: #D8DADF" role="button" id="dropdownMenuLink"
                data-bs-toggle="dropdown" aria-haspopup="true" aria-expanded="false" @class([
                    'rounded-pill bg-light border-dark text-dark btn text-dark px-2 py-2 me-2',
    ...
    @if (!$isUserSame && !$friend)
        <button type="button"
            ...
            class="btn btn-primary rounded-pill text-light px-3 py-2 me-2">
    ...
    @endif
        @if ($isFollowingParticipant)
            <button type="button"
                onclick="formRequestSubmitById(null ,'sendFollowRequest')"
                class="btn btn-success rounded-pill text-dark px-3 py-2 ">
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor"
                    class="bi bi-person-check-fill" viewBox="0 0 16 16">
                    <path fill-rule="evenodd"
                        d="M15.854 5.146a.5.5 0 0 1 0 .708l-3 3a.5.5 0 0 1-.708 0l-1.5-1.5a.5.5 0 0 1 .708-.708L12.5 7.793l2.646-2.647a.5.5 0 0 1 .708 0" />
        ...
        @else
            <button type="button"
                onclick="formRequestSubmitById(null ,'sendFollowRequest')"
                class="btn btn-primary rounded-pill text-light px-3 py-2 ">
                <svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" fill="currentColor"
                    class="bi bi-person-plus me-1" viewBox="0 0 16 16">
                    <path
                        d="M6 8a3 3 0 1 0 0-6 3 3 0 0 0 0 6m2-3a2 2 0 1 1-4 0 2 2 0 0 1 4 0m4 8c0 1-1 1-1 1H1s-1 0-1-1 1-4 6-4 6 3 6 4m-1-.004c-.001-.246-.154-.986-.832-1.664C9.516 10.68 8.289 10 6 10s-3.516.68-4.168 1.332c-.678.678-.83 1.418-.832 1.664z" />
        ...
        @endif
        <button type="button"
            class="btn btn-light rounded-pill border-dark text-dark px-2 py-2 ms-2">
            <svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" fill="currentColor"
                class="bi bi-person-plus me-1" viewBox="0 0 16 16">
                <path
                    d="M6 8a3 3 0 1 0 0-6 3 3 0 0 0 0 6m2-3a2 2 0 1 1-4 0 2 2 0 0 1 4 0m4 8c0 1-1 1-1 1H1s-1 0-1-1 1-4 6-4 6 3 6 4m-1-.004c-.001-.246-.154-.986-.832-1.664C9.516 10.68 8.289 10 6 10s-3.516.68-4.168 1.332c-.678.678-.83 1.418-.832 1.664z" />
