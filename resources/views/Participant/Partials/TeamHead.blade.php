@php
    ...
    if (isset($teamMember[0])) {
        ...
        if ($status == 'rejected' && $teamMember->actor == 'invitee') {
            $status = 'rejected_me';
        }
    } else {
        $status = null;
            ...
            <div class="team-info">
                ...
                <div class="dropdown">
                    ...
                    <div class="dropdown-menu" style="font-size: 14px;" aria-labelledby="dropdownMenuButton">
                        <div>
                            @if (is_null($status))
                                <form class="d-inline text-left w-100 h-100" method="POST" action="{{route('participant.member.pending', ['id' => $selectTeam->id]) }}">
                                    @csrf()
                    ...
                    @if ($selectTeam->creator_id == $user->id)
                        ...
                        @include('Participant.Partials.EditTeamModal')
                    @endif
                </div>
                @endif
            </div>
