...
<div class="position-relative d-block ">
    ...
    <div @class([
        ...
        <a href="{{ route('public.event.view', ['id' => $joinEvent->id, 'title' => bladeSlug($joinEvent->eventName)]) }}">
            <img 
                id="eventBanner"
            ...
            <div class="row mx-0 w-100" style="padding: 5px 10px;">
                <div class="col-6 col-xl-5 d-flex justify-content-start d my-1 px-0">
                    <a class="d-flex w-100 justify-content-start align-items-center"
                        href="{{ route('public.event.view', ['id' => $joinEvent->id, 'title' => bladeSlug($joinEvent->eventName)]) }}">
                ...
                <div onclick="goToUrl(event, this)"
                    data-url="{{ route('public.organizer.view', ['id' => $joinEvent->user->id, 'title' => bladeSlug($joinEvent->user->name)]) }}"
                ...
                <form id="{{ 'followForm' . $joinEvent->id . $random_int }}" method="POST" class="d-none d-xl-flex col-6 followFormProfile col-xl-2 my-2 justify-content-end text-end px-0"
                    ...
                    @guest
                        <button type="button"
                            onclick="reddirectToLoginWithIntened('{{ route('public.organizer.view', ['id' => $joinEvent->user_id, 'title'=> bladeSlug($joinEvent?->user?->name)]) }}')"
