                    ...
                    <ul class="d-flex justify-content-start flex-column align-items-start">
                        @foreach ($joinEvent->roster as $roster)
                            <li onclick="goToUrl(event, this)"
                                data-url="{{ route('public.participant.view', ['id' => $roster->user->id, 'title' => $roster->user->slug ]) }}"
                                class="list-unstyled"
                            >
            ...
            <div class="row mx-0 w-100" style="padding: 5px 10px;">
                ...
                <div onclick="goToUrl(event, this)"
                    ...
                    <div class="text-start d-inline-flex flex-column justify-content-center ">
                        <small class="d-inline-block my-0 text-wrap font-nunito fw-medium  text-primary-dark">{{ truncateText($joinEvent->eventDetails->user->name) }} </small>
                        <small
                            data-count="{{ array_key_exists($joinEvent->eventDetails->user_id, $followCounts) ? $followCounts[$joinEvent->eventDetails->user_id] : 0 }} "
                            class="d-block p-0 font-nunito fw-normal  text-gray-medium {{ 'followCounts' . $joinEvent->eventDetails?->user_id }}">
                ...
                <form onclick="event.stopPropagation(); " 
                    ...
                    method="POST" 
                    class="col-6 d-none d-xl-flex followFormProfile col-xl-2 my-2 justify-content-end align-items-center text-end px-0"
                    ...
                    @guest
                        <button type="button"
                            onclick="event.preventDefault(); event.stopPropagation(); reddirectToLoginWithIntened('{{ route('public.organizer.view', ['id' => $joinEvent->eventDetails?->user_id, 'title' => $joinEvent->eventDetails?->user?->slug ]) }}')"
                    ...
                    @auth
                        @if ($user->role == 'PARTICIPANT')
                            <button type="submit" class="  roster-button  {{ 'followButton' . $joinEvent->eventDetails?->user_id }}  "
