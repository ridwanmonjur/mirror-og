<div>
    ...
    @if ($currentTab == $name)
        <div class="tab-size">
            ...
            @else
                <table id="{{ 'table' . $name }}" style="max-width: 80%; margin: auto;"
                    class="table responsive  table-striped bg-white px-3">
                    <tbody>
                        @foreach ($data as $member)
                            <tr wire:key="{{ $name . $member->id }}" class="st">
                                <td class="colorless-col">
                                    <svg onclick="redirectToProfilePage({{ $member->{$propertyName}->id }});"
